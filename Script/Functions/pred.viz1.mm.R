pred.viz1.mm <- function (cur_pat){
  ...
  source('Script/Functions/pred.viz.mm.R') # visualize mm recovery pathway
  ...
  # 5: add MM predictions
  # cur_pat_dat <- eval_mm %>%
  #   filter(Number == cur_pat) 
  # 
  # predtimes <- seq(max(cur_pat_dat$Days), 180, length.out = 100) # time vector to provide predictions and PIs for
  # 
  ...
  # 
  # lb <- round(Indv_pred_mm$low[length(Indv_pred_mm$low)],2)
  # ub <- round(Indv_pred_mm$upp[length(Indv_pred_mm$upp)],2)
  # 
  # p <- p + 
  ...
  #   geom_ribbon(data = filter(Indv_pred_mm, type == "pred"), aes(x = Days, y = pred, ymin = low, ymax = upp), fill = "#21918c", alpha = 0.5)
