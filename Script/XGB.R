...
source('Script/Functions/Selles_MM.R') # running Selles mixed models
...
source('Script/Functions/mm.learn.curve.R') # learn curve creation for mm
...
#load("V:\\Prive\\Gun, Govert van der\\Data\\Data_raw.RData")
...
# spaghetti plot with ARAT evolution
...
# spaghetti plot of patients in test
dat_plot <- dat_mod %>% 
  filter(Number %in% test_xgb$Number) %>% 
  group_by(Number) %>% 
  mutate(ARAT_base = factor(case_when(min(ARAT) < 10 ~ "L",
                                min(ARAT) >= 10 & min(ARAT) < 30 ~ "M",
                                min(ARAT) >= 30 ~ "H"),levels = c("L","M","H"))) %>%
  mutate(ARAT_out = factor(case_when(max(ARAT) < 20 ~ "L",
                                 max(ARAT) >= 20 & max(ARAT) < 40 ~ "M",
                                 max(ARAT) >= 40 ~ "H"), levels = c("L","M","H"))) %>% 
  mutate(SAFE = if_else((SA.0 == 1 | SA.9 == 1 | SA.14 == 1) & (FE.none == 1 | FE.partial == 1),0,1)) %>% 
  ungroup()

ggplot(dat_plot) +
  geom_line(aes(x = Days, y = ARAT, group = Number, color = ARAT_out), linewidth = 0.5) +
  geom_point(aes(x = Days, y = ARAT, color = ARAT_out), size = 2) +
  labs(x = "Days since onset", y = "ARAT") +
  scale_x_continuous(breaks = seq(0, 300, by = 50), expand = c(.01,.01)) +
  scale_y_continuous(breaks = seq(0, 60, by = 10), limits = c(0,57), expand = c(.01,.01)) +
...
ggplot() +
  geom_line(data = subset(dat_plot, SAFE==1), aes(x = Days, y = ARAT, group = Number, color = "#5ec962"), linewidth = 0.5) +
  geom_point(data = subset(dat_plot, SAFE==1), aes(x = Days, y = ARAT, color = "#5ec962"), size = 2) +
  geom_line(data = subset(dat_plot, SAFE==0), aes(x = Days, y = ARAT, group = Number, color = "black"), linewidth = 0.5) +
  geom_point(data = subset(dat_plot, SAFE==0), aes(x = Days, y = ARAT, color = "black"), size = 2) +
  labs(x = "Days since onset", y = "ARAT") +
  scale_x_continuous(breaks = seq(0, 300, by = 50), expand = c(.01,.01)) +
  scale_y_continuous(breaks = seq(0, 60, by = 10), limits = c(0,57), expand = c(.01,.01)) +
  coord_cartesian(xlim= c(0,200)) +
  scale_color_manual(name = "Color",
                    values = c("#5ec962" = "#5ec962", "black" = "black"),
                    labels = c("#5ec962" = "SA > 14 & FE > partial", "black" = "SA <= 14 & FE <= partial")) +
  fancy


# jittered box plots for performance metrics
plots <- box.plot.jitter(perf, ID = 'Number', y_lim = c(0,60))
plots[[1]] <- plots[[1]] + labs(title = "XGBoost", y = "Absolute Error")
plots[[2]] <- plots[[2]] + labs(title = "Mixed Model", y = "")
...
perf_wks_xgb %>%
  ...
  ggplot(aes(x=myaxis, y=PIw, fill=time)) +
  geom_boxplot() +
  labs(y = "Absolute Error",
       x = "") +
  scale_y_continuous(lim =c(0,60)) +
...
labels <- c(L = paste("Baseline ARAT [0-10) \n n=",  sample_size2$count[sample_size2$ARAT_base == "L"]),
            ...
            H = paste("Baseline ARAT [30-57] \n n=", sample_size2$count[sample_size2$ARAT_base == "H"]))

ggplot(plot_week, aes(y=AE_xgb)) +
  geom_boxplot() +
...
sample_size3 <- plot_week %>% group_by(ARAT_out) %>% summarise(count=n())
labels <- c(L = paste("Outcome ARAT [0-20) \n n=", sample_size3$count[sample_size3$ARAT_out == "H"]),
            M = paste("Outcome ARAT [20-40) \n n=", sample_size3$count[sample_size3$ARAT_out == "H"]), 
            H = paste("Outcome ARAT [30-57] \n n=", sample_size3$count[sample_size3$ARAT_out == "H"]))
ggplot(plot_week, aes(y = AE_xgb)) +
  geom_boxplot() +
...
# per group & per week prediction uncertainty
perf_wks %>%
  left_join(sample_size5, by = c("time", "ARAT_base")) %>%
  mutate(myaxis = factor(paste0(time, "\n", "n=", count),
                         levels = unique(paste0(time, "\n", "n=", count)))) %>%
  ...
  facet_wrap(~ ARAT_base, scales = "free",
             labeller = labeller(ARAT_base = labels)) +
  labs(y = "Prediction Interval Width",
       x = "") +
  scale_y_continuous(lim =c(0,60))  +
  scale_fill_viridis(discrete = T) +
  fancy +
...
# plot recovery profile of prediction outliers
dat_mod %>%
  filter(Number %in% plots$AE_xgb$outliers$Number) %>% 
  ggplot(aes(x = Days, y = ARAT, group = Number)) +
  geom_point() +
  geom_line() +
  labs(
    x = "Days since onset",
    y = "ARAT",
    title = "Prediction Outliers"
  ) +
  scale_x_continuous(breaks = seq(0, 300, by = 50), expand = c(.01,.01)) +
  scale_y_continuous(breaks = seq(0, 60, by = 10), limits = c(0,57), expand = c(.01,.01)) +
  scale_color_viridis(discrete=T) +
...
lc_dat_mm <- mm.learn.curve(train_mm, # train data
                            test_mm, # test data
                            Tvarn = "Days", # name of Time var
                            IDvarn = "Number", # name of ID var
                            seed = seed
                            )
...
# 
# # for XGboost
# dat_shap_xgb <- select(dat_mod_xgb,-c(Number,last_Days,last_ARAT,all_of(catvars)))
# colnames <- colnames(dat_shap_xgb)
# shapvalsxgb <- shap.values(M_xgb$finalModel, X_train = as.matrix(dat_shap_xgb)) # get shap values from xgb model
# 
...
# dat_shap_xgb_plot <- select(dat_mod_xgb,-c(Number,last_Days,last_ARAT,all_of(dummynames)))
# svobjxgb <- shapviz(data.matrix(shapvalsxgb), X = dat_shap_xgb_plot)
# 
# # plot beeswarm
# sv_importance(svobjxgb, kind = "beeswarm") +
#   labs(
#     title = "XGBoost"
#   )
